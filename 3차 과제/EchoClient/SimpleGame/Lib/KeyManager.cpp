...
std::unique_ptr<KeyInput> g_key_input { std::make_unique<KeyInput>() };

KeyInput::KeyInput() : is_key_down { }, is_key_up { }
{
}
...
// 한번만 눌리는지...
bool KeyInput::key_down( int virtualKey )
{
...
// 한번 눌럿다가 띄어지는지 체크...
bool KeyInput::key_up( int virtualKey )
{
...
bool KeyInput::toggle_key( int virtualKey )
{
...
bool KeyInput::key_stay( int virtualKey )
{
	if (GetAsyncKeyState(virtualKey) & 0x8000)
		return true;
