...
protected:
	...
	HANDLE*						io_handler;					// 8
	ErrorCode					custom_last_server_error;	// 8  user
	ErrorCode					custom_last_os_error;		// 8  kernel
	...
	ConcurrentStack<uint64_t, 5000>	id_pool;

	alignas(CACHE_LINE)	int64_t		total_recv_bytes;			// 8
	alignas(CACHE_LINE)	int64_t		total_recv_count;			// 8
	alignas(CACHE_LINE)	int64_t		total_sent_bytes;			// 8
	alignas(CACHE_LINE)	int64_t		total_sent_count;			// 8
	alignas(CACHE_LINE)	uint64_t	current_accepted_count;		// 8

	static thread_local TimeTaskScheduler* local_timer;
